...
if __name__ == '__main__':
    ...
    #process 0 is going to calculate and send to process 1
    if(rank == 0):
        ...
        while(True):
            ...
            t2 = time.time()
            print(f"Im 0 ->  {diff}")
            req = comm.isend(grid, dest=1, tag=2) #Envoie les données a l'autre processus
            req.wait()
            req = comm.isend(diff, dest=1, tag=1) #Envoie les données a l'autre processus
            req.wait()
