...
#echanger deux a deux:
if(rank == 0):
    comm.send(enveloppe, dest=1, tag=1) 
    other_envelop = comm.recv(source=1, tag=1)
else:
    other_envelop = comm.recv(source=0, tag=1)
    comm.send(enveloppe, dest=0, tag=1)
...
enveloppe = np.array(other_envelop.tolist() + enveloppe.tolist())
